...
import com.tracksure.android.model.TrackSureFilePacket
...
class MediaSendingManager(
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val filePacket = TrackSureFilePacket(
                fileName = file.name,
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.tracksure.android.features.file.FileUtils.getMimeTypeFromExtension(file.name)
            } catch (_: Exception) { 
    ...
    private fun sendPrivateFile(
        toPeerID: String,
        filePacket: TrackSureFilePacket,
        filePath: String,
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.tracksure.android.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.tracksure.android.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.tracksure.android.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.tracksure.android.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
