...
import com.tracksure.android.ui.theme.BASE_FONT_SIZE
...
fun PeopleSection(
    ...
    Column(modifier = modifier) {
        ...
        sortedPeers.forEach { pid ->
            ...
            val (b, _) = com.tracksure.android.ui.splitSuffix(dn)
            if (b != "You") baseNameCounts[b] = (baseNameCounts[b] ?: 0) + 1
        }

        // Offline favorites (exclude ones mapped to connected)
        val offlineFavorites = com.tracksure.android.favorites.FavoritesPersistenceService.shared.getOurFavorites()
        ...
        sortedPeers.forEach { peerID ->
            ...
            val (bName, _) = com.tracksure.android.ui.splitSuffix(displayName)
        ...
        offlineFavorites.forEach { fav ->
            ...
            val nostrConvKey: String? = try {
                val npubOrHex = com.tracksure.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(fav.peerNoisePublicKey)
                if (npubOrHex != null) {
                    val hex = if (npubOrHex.startsWith("npub")) {
                        val (hrp, data) = com.tracksure.android.nostr.Bech32.decode(npubOrHex)
            ...
            val dn = peerNicknames[favPeerID] ?: fav.peerNickname
            val (bName, _) = com.tracksure.android.ui.splitSuffix(dn)
            val showHash = (baseNameCounts[bName] ?: 0) > 1
...
private fun PeerItem(
    ...
    val (baseNameRaw, suffixRaw) = com.tracksure.android.ui.splitSuffix(displayName)
