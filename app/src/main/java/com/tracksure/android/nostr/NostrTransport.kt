package com.tracksure.android.nostr

import android.content.Context
import android.util.Log
import com.tracksure.android.model.ReadReceipt
import com.tracksure.android.model.NoisePayloadType
...
class NostrTransport(
        ...
        transportScope.launch {
            try {
                ...
                val recipientPeerIDForEmbed = try {
                    com.tracksure.android.favorites.FavoritesPersistenceService.shared
                ...
                val embedded = NostrEmbeddedTrackSure.encodePMForNostr(
                    content = content,
                    messageID = messageID,
    ...
    private fun sendNextReadAck() {
        ...
        transportScope.launch {
            try {
                ...
                val ack = NostrEmbeddedTrackSure.encodeAckForNostr(
    ...
    fun sendDeliveryAckGeohash(
        ...
        transportScope.launch {
            try {
                ...
                val embedded = NostrEmbeddedTrackSure.encodeAckForNostrNoRecipient(
                    type = NoisePayloadType.DELIVERED,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                )
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.tracksure.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.tracksure.android.geohash.ChannelID.Location) {
        ...
        transportScope.launch {
            try {
                ...
                val embedded = NostrEmbeddedTrackSure.encodePMForNostrNoRecipient(
                    content = content,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                ) ?: run {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.tracksure.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.tracksure.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.tracksure.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
