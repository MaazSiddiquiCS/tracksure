...
/**
 * TrackSureFilePacket: TLV-encoded file transfer payload for BLE mesh.
 * TLVs:
 *  - 0x01: filename (UTF-8)
 ...
 */
data class TrackSureFilePacket(
    val fileName: String,
    val fileSize: Long,
    val mimeType: String,
    ...
    fun encode(): ByteArray? {
        try {
            android.util.Log.d("TrackSureFilePacket", "🔄 Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
                android.util.Log.e("TrackSureFilePacket", "❌ TLV field too large: name=${nameBytes.size}, mime=${mimeBytes.size} (max: 65535)")
                return null
            }
            if (content.size > 0xFFFF) {
                android.util.Log.d("TrackSureFilePacket", "📦 Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                android.util.Log.d("TrackSureFilePacket", "📏 TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            android.util.Log.d("TrackSureFilePacket", "✅ Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
            android.util.Log.e("TrackSureFilePacket", "❌ Encoding failed: ${e.message}", e)
            return null
        }
    }

    companion object {
        fun decode(data: ByteArray): TrackSureFilePacket? {
            android.util.Log.d("TrackSureFilePacket", "🔄 Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                val result = TrackSureFilePacket(n, s, m, c)
                android.util.Log.d("TrackSureFilePacket", "✅ Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {
                android.util.Log.e("TrackSureFilePacket", "❌ Decoding failed: ${e.message}", e)
                return null
            }
        }
    }
