...
import com.tracksure.android.core.ui.utils.singleOrTripleClickable
...
fun TorStatusIcon(
    ...
    val torStatus by com.tracksure.android.net.TorManager.statusFlow.collectAsState()
    
    if (torStatus.mode != com.tracksure.android.net.TorMode.OFF) {
        val cableColor = when {
...
fun PeerCounter(
    ...
    isConnected: Boolean,
    selectedLocationChannel: com.tracksure.android.geohash.ChannelID?,
    ...
    Row(
        ...
        Icon(
            ...
            contentDescription = when (selectedLocationChannel) {
                is com.tracksure.android.geohash.ChannelID.Location -> "Geohash participants"
...
private fun PrivateChatHeader(
    ...
    val isMutualFavorite = remember(peerID, peerNicknames) {
        try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                com.tracksure.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.isMutual == true
            } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                com.tracksure.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.isMutual == true
            } else false
    ...
    val titleText: String = if (isNostrDM) {
        ...
        val (conversationGeohash, baseName) = try {
            val repoField = com.tracksure.android.ui.GeohashViewModel::class.java.getDeclaredField("repo")
            repoField.isAccessible = true
            val repo = repoField.get(viewModel.geohashViewModel) as com.tracksure.android.nostr.GeohashRepository
            val gh = repo.getConversationGeohash(peerID) ?: "geohash"
            val fullPubkey = com.tracksure.android.nostr.GeohashAliasRegistry.get(peerID) ?: ""
        ...
        peerNicknames[peerID] ?: run {
            val titleFromFavorites = try {
                if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    val noiseKeyBytes = peerID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                    com.tracksure.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.peerNickname
                } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    com.tracksure.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.peerNickname
                } else null
...
private fun MainHeader(
    ...
    val bookmarksStore = remember { com.tracksure.android.geohash.GeohashBookmarksStore.getInstance(context) }
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(end = 14.dp)) {
                ...
                val currentGeohash: String? = when (val sc = selectedLocationChannel) {
                    is com.tracksure.android.geohash.ChannelID.Location -> sc.channel.geohash
                    else -> null
                }
...
private fun LocationChannelsButton(
    ...
    val (badgeText, badgeColor) = when (selectedChannel) {
        ...
        is com.tracksure.android.geohash.ChannelID.Location -> {
            val geohash = (selectedChannel as com.tracksure.android.geohash.ChannelID.Location).channel.geohash
