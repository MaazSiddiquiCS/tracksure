...
class NostrDirectMessageHandler(
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (packet.type != com.tracksure.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch

                val noisePayload = com.tracksure.android.model.NoisePayload.decode(packet.payload) ?: return@launch
                val messageTimestamp = Date(giftWrap.createdAt * 1000L)
                val convKey = "nostr_${senderPubkey.take(16)}"
                repo.putNostrKeyMapping(convKey, senderPubkey)
                com.tracksure.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        payload: com.tracksure.android.model.NoisePayload,
        convKey: String,
        ...
        when (payload.type) {
            com.tracksure.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                val pm = com.tracksure.android.model.PrivateMessagePacket.decode(payload.data) ?: return
            ...
            com.tracksure.android.model.NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = com.tracksure.android.model.TrackSureFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.tracksure.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
