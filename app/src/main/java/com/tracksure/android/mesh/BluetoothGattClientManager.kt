...
import com.tracksure.android.ui.debug.DebugSettingsManager
import com.tracksure.android.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    fun start(): Boolean {
        ...
        try {
            if (!com.tracksure.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value) {
    ...
    fun onScanStateChanged(shouldScan: Boolean) {
        val enabled = try { com.tracksure.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
